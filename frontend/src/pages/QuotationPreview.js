...
// INCOTERMS Labels (Kod → Kısa Ad) - PDF için
...
// Shipping term label ve açıklama al
const getShippingTermInfo = (term, lang) => {
  if (!term) return { label: "", description: "" };
  const langKey = lang === "english" ? "en" : "tr";
  
  if (INCOTERMS_LABELS[term]) {
    return {
      label: `${term} — ${INCOTERMS_LABELS[term][langKey]}`,
      description: INCOTERMS_DESCRIPTIONS[term][langKey]
    };
  }
  if (FREIGHT_TERMS[term]) {
    return {
      label: langKey === "tr" ? FREIGHT_TERMS[term].trLabel : FREIGHT_TERMS[term].enLabel,
      description: langKey === "tr" ? FREIGHT_TERMS[term].trDesc : FREIGHT_TERMS[term].enDesc
    };
  }
  return { label: term, description: "" };
};
...
const QuotationPreview = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      toast.loading('Görseller yükleniyor...', { id: toastId });
      ...
      const coverEl = document.querySelector('[data-testid="proposal-cover"]');
      if (coverEl) {
        toast.loading('Kapak sayfası oluşturuluyor...', { id: toastId });
        ...
        pdf.addImage(
          ...
          0,
          coverImgWidth,
          Math.min(coverImgHeight, pageHeight)
        );
      }

      const contentElements = document.querySelectorAll('.print-content');

      if (contentElements && contentElements.length > 0) {
        toast.loading('İçerik sayfaları oluşturuluyor...', { id: toastId });
        ...
        for (let i = 0; i < contentElements.length; i++) {
          ...
          const contentImgWidth = pageWidth;
          const contentImgHeight =
            (contentCanvas.height * pageWidth) / contentCanvas.width;
          ...
          while (remainingHeight > 0) {
            if (!isFirstChunk && remainingHeight > 10) {
              pdf.addPage();
            }
            isFirstChunk = false;
      ...
      const fileName = `Teklif-${quotation.quote_no}-${quotation.customer_name}.pdf`;
      pdf.save(fileName);

      toast.success('PDF başarıyla indirildi!', { id: toastId });
    } catch (error) {
      console.error('PDF export error:', error);
      toast.error('PDF oluşturulamadı. Print dialog açılıyor...', { id: toastId });
  ...
  // Düzenlenmiş PDF'i önizle (yeni sekmede aç)
  const handlePreviewEditedPdf = async () => {
    try {
      const response = await axios.get(`${API}/quotations/${id}/download-edited-pdf`, {
        responseType: 'blob'
      });
      ...
      toast.success('Düzenlenmiş PDF yeni sekmede açıldı!');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* TOP BAR - 2 Sıralı Kompakt Header */}
      <div className="border-b border-border/40 bg-card/95 backdrop-blur-sm sticky top-0 z-50 print:hidden shadow-sm">
        <div className="max-w-[210mm] mx-auto px-4 py-2">
          {/* Üst Satır: Geri ve Başlık */}
          <div className="flex items-center justify-center gap-2 mb-2">
            <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => navigate(`/quotations/${type}`)}>
              <ArrowLeft className="h-4 w-4" />
            </Button>
            <h1 className="text-sm font-bold">{quotation.quote_no}</h1>
          </div>
          
          {/* Alt Satır: Tüm Butonlar - Ortalanmış */}
              ...
              {quotation?.has_edited_pdf && (
                <>
                  <Button 
                    variant="ghost"
                    size="sm"
                    onClick={handlePreviewEditedPdf}
                      ...
                      {quotation.line_items
                        ...
                        .map((item, index) => (
                          <tr key={index} className="border-b border-gray-200 align-top">
                            ...
                            <td className="py-3">
                              <div className="font-bold text-gray-900" style={{ fontSize: '14px' }}>{item.item_short_name}</div>
                              {item.variant_sku && (
                                <div className="text-xs text-gray-600 font-mono mt-1" style={{ fontSize: '11px' }}>
                                  SKU: {item.variant_sku}
                                </div>
                              )}
                              {item.item_description && (
                                <div className="text-gray-700 mt-1" style={{ fontSize: '12px', lineHeight: '1.4' }}>
                                  {item.item_description}
                                </div>
                              )}
                            </td>
                ...
                <div className="border-t-2 border-gray-300 pt-4 space-y-2">
                  {Object.keys(quotation.totals_by_currency?.totals || {}).map(
                    (currency) => {
                      ...
                      // Import cost (indirimden sonra eklenir)
                      let importCost = 0;
                      if (
                        quotation.is_international &&
                        quotation.import_extra_cost_amount > 0 &&
                        quotation.import_extra_cost_currency === currency
                      ) {
                        importCost = quotation.import_extra_cost_amount;
                      }

                      // Genel toplam = (Ara toplam - İndirim) + Import Cost
                      const finalTotal = subtotal - discountAmount + importCost;
                      ...
                      return (
                        <div key={currency}>
                          ...
                          {/* Import/Freight/Customs Cost (indirimden sonra) */}
                          {importCost > 0 && (
                            <div className="flex justify-between items-center py-1">
                              <span
                                className="font-semibold text-gray-700"
                              ...
                              >
                                {quotation.language === 'english'
                                  ? 'Import / Freight / Customs Cost'
                                  : 'İthalat / Navlun / Gümrük Masrafı'}:
                              </span>
                              <span
                                className="font-bold text-blue-600"
                                style={{ fontSize: '14px' }}
                              >
                                +{formatCurrency(importCost, currency)}
                              </span>
                            </div>
                          )}
              ...
              <div className="space-y-4 text-sm">
                ...
                {quotation.delivery_terms && (
                  <div>
                    ...
                    {/* Alt alta liste formatında göster */}
                    <ul className="text-gray-700 space-y-1 ml-4">
                      {quotation.delivery_terms.split('\n').filter(line => line.trim()).map((line, idx) => (
                ...
                {/* Shipping Terms (Incoterms) - PDF'de açıklamasıyla */}
                {quotation.shipping_term && (
                  <div>
                    <div
                      style={{
                        borderLeft: '5px solid #dc2626',
                    ...
                    >
                      <h4 className="font-semibold" style={{ color: '#991b1b' }}>
                        {quotation.language === 'english'
                          ...
                          : 'Teslimat Şekli (Incoterms):'}
                      </h4>
                    </div>
                    <p className="text-gray-700">
                      {getShippingTermInfo(quotation.shipping_term, quotation.language).label}
                    </p>
                    {getShippingTermInfo(quotation.shipping_term, quotation.language).description && (
